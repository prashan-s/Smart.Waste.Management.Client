    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 bg-white shadow-lg py-5 md:py-6 flex justify-around">
            ...
            <button onClick={() => navigate('/client/profile')} className="flex flex-col items-center">
                <LuUser2 size={30} className={isActive('/client/profile') ? "text-black" : "text-gray-500"} />
                <span className={`text-xs ${isActive('/client/profile') ? "text-black" : "text-gray-500"}`}>Profile</span>
            </button>
        </div>
    );
};
